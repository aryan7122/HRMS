...
const JobForm = ({ onSubmit }) => {
  ...
  const {
    ...
    ref: leaveTypeRef,
    buttonRef: leaveTypeButtonRef,
    handleToggle: toggleLeaveType,
    setIsOpen: setLeaveTypeOpen,
  } = OutsideClick();
  const {
    isOpen: isTypeOpen,
    ref: typeRef,
  ...
  const {
    isOpen: isEmployeeNameOpen,
    ref: EmployeeNameRef,
    buttonRef: EmployeeNameButtonRef,
    handleToggle: toggleEmployeeName,
    setIsOpen: setEmployeeNameOpen,
  } = OutsideClick();
  ...
  const handleStartDateChange = (date) => {
    ...
    setFormData({
      ...formData,
      fromDate: date,
      totalDays: calculateDaysDifference(date, endDate),
    });
  };
  ...
  const handleEndDateChange = (date) => {
    ...
    setFormData({
      ...formData,
      toDate: date,
      totalDays: calculateDaysDifference(startDate, date),
    });
  };
  ...
  useEffect(() => {
    ...
    if (fromDate && toDate) {
      const totalDays = calculateDaysDifference(fromDate, toDate);
  ...
  useEffect(() => {
    axios
      .post(
        `https://hrms.dragnilifecare.in/public/api/leave/list`,
        {},
        {
          headers: {
      ...
      .then((response) => {
        ...
        setFormData((prevFormData) => ({
          ...
          leaveType: leaveForm.leave_type_name || "",
          fromDate: leaveForm.from_date || "",
          toDate: leaveForm.to_date || "",
          totalDays: leaveForm.days || "",
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post(
        "https://hrms.dragnilifecare.in/public/api/leave/create/update",
        requestData,
        {
          headers: {
  ...
  useEffect(() => {
    ...
    axios
      .post(
        "https://hrms.dragnilifecare.in/public/api/employee/list",
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  useEffect(() => {
    ...
    axios
      .post(
        "https://hrms.dragnilifecare.in/public/api/leave/master/list",
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleSearchQueryChangeLeaveType = (e) =>
    setSearchQueryLeaveType(e.target.value);
  const handleSearchQueryChangeType = (e) => setSearchQueryType(e.target.value);
  const handleSearchQueryChangeEmployeeName = (e) =>
    setSearchQueryEmployeeName(e.target.value);
          ...
          <div id="form">
            <div className="from1">
              <div className="form-group">
                ...
                <div className="dropdown">
                  <div
                    ...
                    ref={EmployeeNameButtonRef}
                    onClick={toggleEmployeeName}>
                    <div className="divselect">
                      {formData.employeeName || "Select Employee Name"}
                    </div>
                        ...
                        {departmentHead
                          .filter((option) =>
                            `${option.first_name} ${option.last_name}`
                              .toLowerCase()
                              .includes(searchQueryEmployeeName.toLowerCase())
                          )
                          .map((option) => (
                            <div
                              className="dropdown-item"
                              onClick={() =>
                                selectOption("employeeName", option)
                              }
                              key={option.user_id}>
                              {option.first_name} {option.last_name}
                            </div>
                        ...
                        {leaveTypeData
                          .filter((option) =>
                            `${option.leave_type} ${option.leave_type}`
                              .toLowerCase()
                              .includes(searchQueryLeaveType.toLowerCase())
                          )
                          .map((option) => (
                            <div
                              className="dropdown-item"
              ...
              <DatePicker
                ...
                initialDate={formData.fromDate}
              />
              <DatePicker
                label="To Date"
                onDateChange={handleEndDateChange}
                initialDate={formData.toDate}
              />
              <div className="form-group">
              ...
              <div className="form-group">
                ...
                <div className="dropdown">
                  <div
                    ...
                    <span id="toggle_selectIcon">
                      {" "}
                      {!isTypeOpen ? <IoIosArrowDown /> : <IoIosArrowUp />}{" "}
                    </span>
                  </div>
                  {isTypeOpen && (
                      ...
                      <div className="dropdown_I">
                        {["Paid", "Unpaid", "On Duty"]
                          .filter((option) =>
                            option
                              .toLowerCase()
                              .includes(searchQueryType.toLowerCase())
                          )
                          .map((option) => (
                            <div
                              className="dropdown-item"
